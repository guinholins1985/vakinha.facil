...
const LogoIcon = () => (
    // FIX: Changed strokeWidth to be a string.
...
const getInitialVaquinhas = () => ([
    {
        ...
        status: "Ativa",
        participants: [
            { id: 1, name: "João", status: "Pago" },
            { id: 2, name: "Maria", status: "Atraso" },
            { id: 3, name: "Carlos", status: "Pago" },
            { id: 4, name: "Ana", status: "Pendente" },
        ],
        payments: { pix: 1000, card: 500, boleto: 0 },
        history: [ { month: "Jan", amount: 500 }, { month: "Fev", amount: 500 }, { month: "Mar", amount: 500 } ]
    },
    {
        id: 2,
        ...
        deadline: "Finalizado",
        status: "Finalizada",
        participants: Array(24).fill(0).map((_, i) => ({ id: i+1, name: `Convidado ${i+1}`, status: 'Pago'})),
    ...
    },
    {
        ...
        deadline: "1/1 mês",
        status: "Ativa",
        participants: Array(18).fill(0).map((_, i) => ({ id: i+1, name: `Amigo ${i+1}`, status: i % 2 === 0 ? 'Pago' : 'Pendente' })),
        payments: { pix: 450, card: 0, boleto: 0 },
        history: [{ month: "Dez", amount: 450 }]
    }
]);

type Vaquinha = ReturnType<typeof getInitialVaquinhas>[0];
type Participant = Vaquinha['participants'][0];

const VaquinhaDetail = ({ vaquinha, onBack, onEdit, onUpdateParticipants }: { vaquinha: Vaquinha, onBack: () => void, onEdit: (v: Vaquinha) => void, onUpdateParticipants: (id: number, p: Participant[]) => void }) => {
    ...
    const handleAddParticipant = () => {
        ...
        const newParticipant = { id: Date.now(), name: participantName, status: 'Pendente' };
        const updatedParticipants = [...vaquinha.participants, newParticipant];
        onUpdateParticipants(vaquinha.id, updatedParticipants);
        addToast(`"${participantName}" foi adicionado.`);
        setParticipantName('');
    };
    
    const handleRemoveParticipant = (participantId: number) => {
        const updatedParticipants = vaquinha.participants.filter(p => p.id !== participantId);
        onUpdateParticipants(vaquinha.id, updatedParticipants);
        addToast('Participante removido.', 'error');
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-slate-50 p-4 rounded-lg">
                     ...
                     <div className="bg-slate-50 p-4 rounded-lg">
                        ...
                        <div className="flex flex-col space-y-2">
                             <button onClick={() => addToast('Lembretes enviados!')} className="text-left w-full bg-white hover:bg-slate-100 border border-gray-200 text-slate-800 font-semibold py-2 px-4 rounded-lg transition-colors text-sm">Enviar lembrete</button>
                             <button onClick={() => addToast('Solicitação enviada para análise.')} className="text-left w-full bg-white hover:bg-slate-100 border border-gray-200 text-slate-800 font-semibold py-2 px-4 rounded-lg transition-colors text-sm">Solicitar distribuição</button>
                             <button onClick={() => onEdit(vaquinha)} className="text-left w-full bg-white hover:bg-slate-100 border border-gray-200 text-slate-800 font-semibold py-2 px-4 rounded-lg transition-colors text-sm">Editar vaquinha</button>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-slate-50 p-4 rounded-lg h-full flex flex-col">
                        <h3 className="font-bold text-gray-700 font-heading mb-3">PARTICIPANTES ({vaquinha.participants.length})</h3>
                                ...
                                {vaquinha.participants.map((p: any) => (
                                    <li key={p.id} className="flex justify-between items-center group">
                                        ...
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <button onClick={() => handleRemoveParticipant(p.id)} className="opacity-0 group-hover:opacity-100 text-red-500 hover:text-red-700 transition-opacity">
                        ...
                        <div className="flex space-x-2 mt-3">
                           <input type="text" value={participantName} onChange={e => setParticipantName(e.target.value)} placeholder="Nome do participante..." className="flex-grow border rounded-md px-3 py-1.5 text-sm"/>
                           <button onClick={handleAddParticipant} className="text-sm bg-emerald-100 hover:bg-emerald-200 text-emerald-800 font-semibold py-2 px-4 rounded-lg transition-colors">+ Adicionar</button>
                        </div>
                    </div>
                </div>
            </div>
...
const VaquinhaList = ({ vaquinhas, onSelectVaquinha, onAddNew }: { vaquinhas: Vaquinha[], onSelectVaquinha: (vaquinha: Vaquinha) => void, onAddNew: () => void }) => {
    // FIX: Get addToast from useToast hook.
    const { addToast } = useToast();
    ...
    return (
        <>
            <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={onAddNew} className="bg-emerald-500 text-white font-semibold px-5 py-2 rounded-lg shadow-md hover:bg-emerald-600 transition-all duration-300 flex items-center space-x-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {vaquinhas.map((vaquinha) => {
                    ...
                    return (
                        <div key={vaquinha.id} className="bg-white rounded-xl shadow-lg p-6 flex flex-col justify-between hover:shadow-xl transition-shadow">
                            <div>
                            ...
                            <div className="flex space-x-3 text-sm">
                                ...
                                <button onClick={() => {navigator.clipboard.writeText(`Convite para ${vaquinha.title}!`); addToast('Link de convite copiado!')}} className="flex-1 bg-emerald-100 hover:bg-emerald-200 text-emerald-800 font-semibold py-2 px-4 rounded-lg transition-colors">Convidar</button>
                            </div>
                        </div>
                    );
                })}
...
const VaquinhaFormModal = ({ vaquinha, onSave, onClose }: { vaquinha?: Vaquinha | null, onSave: (data: Vaquinha) => void, onClose: () => void }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        const finalData = {
            id: vaquinha?.id || Date.now(),
            title: formData.title,
            goal: Number(formData.goal) || 0,
            deadline: formData.deadline,
            collected: vaquinha?.collected || 0,
            status: vaquinha?.status || 'Ativa',
            participants: vaquinha?.participants || [],
            payments: vaquinha?.payments || { pix: 0, card: 0, boleto: 0 },
            history: vaquinha?.history || [],
        };
        onSave(finalData);
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
                <div className="flex justify-between items-center p-4 border-b">
                    <h2 className="text-xl font-bold">{vaquinha ? 'Editar' : 'Criar'} Vaquinha</h2>
                    <button onClick={onClose} className="text-gray-500 hover:text-gray-800 text-3xl">&times;</button>
                </div>
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                     <div>
                        <label className="block text-sm font-medium text-gray-700">Título da Vaquinha</label>
                     ...
                     <div className="grid grid-cols-2 gap-4">
                         <div>
                            ...
                            <input type="number" name="goal" value={formData.goal} onChange={handleChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3" required />
                        </div>
                         <div>
                            <label className="block text-sm font-medium text-gray-700">Prazo</label>
                            <input type="text" name="deadline" value={formData.deadline} onChange={handleChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3" placeholder="Ex: 30 dias" required />
                        </div>
                    </div>
                    <div className="flex justify-end pt-4">
                        <button onClick={onClose} type="button" className="bg-white py-2 px-4 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">Cancelar</button>
                        <button type="submit" className="ml-3 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-emerald-600 hover:bg-emerald-700">Salvar</button>
                    </div>
                </form>
            </div>
        </div>
...
const GroupAdminDashboard = () => {
    const { addToast } = useToast();
    const [vaquinhas, setVaquinhas] = useState<Vaquinha[]>(getInitialVaquinhas());
    ...
    const handleSaveVaquinha = (data: Vaquinha) => {
        if (editingVaquinha) {
            setVaquinhas(prev => prev.map(v => v.id === data.id ? data : v));
            addToast('Vaquinha atualizada com sucesso!');
        } else {
            setVaquinhas(prev => [data, ...prev]);
            addToast('Vaquinha criada com sucesso!');
        }
        setIsFormVisible(false);
        setEditingVaquinha(null);
    };

    const handleEditVaquinha = (v: Vaquinha) => {
        setEditingVaquinha(v);
        setIsFormVisible(true);
    };
    
    const handleUpdateParticipants = (vaquinhaId: number, participants: Participant[]) => {
        setVaquinhas(prev => prev.map(v => v.id === vaquinhaId ? { ...v, participants } : v));
        setSelectedVaquinha(prev => prev ? { ...prev, participants } : null);
    };
    
    return (
        <div className="bg-slate-100 min-h-screen">
            {isFormVisible && <VaquinhaFormModal vaquinha={editingVaquinha} onSave={handleSaveVaquinha} onClose={() => { setIsFormVisible(false); setEditingVaquinha(null); }} />}
            <div className="pt-24">
                <main className="container mx-auto px-6 py-8">
                   {selectedVaquinha ? (
                       <VaquinhaDetail 
                            vaquinha={selectedVaquinha} 
                            onBack={() => setSelectedVaquinha(null)} 
                            onEdit={handleEditVaquinha}
                            onUpdateParticipants={handleUpdateParticipants}
                        />
                   ) : (
                       <VaquinhaList vaquinhas={vaquinhas} onSelectVaquinha={setSelectedVaquinha} onAddNew={() => setIsFormVisible(true)} />
                   )}
                </main>
            </div>
        </div>
...
const getInitialAdminData = () => ({
    ...
    transactions: [
        { id: 1, date: '2024-02-01', value: 350, type: 'Taxa Fixa' },
        { id: 2, date: '2024-03-15', value: 890, type: 'Taxa %' },
        { id: 3, date: '2024-04-10', value: 450, type: 'Assinatura' },
        { id: 4, date: '2024-05-20', value: 1250, type: 'Taxa %' },
        { id: 5, date: '2024-06-05', value: 780, type: 'White-Label' },
        { id: 6, date: '2024-07-18', value: 1530, type: 'Taxa %' },
    ],
    licensees: [
...
const PieChart = ({ data }: { data: { name: string, value: number, color: string }[]}) => {
    ...
    return (
        <div className="flex items-center">
            <svg viewBox="0 0 100 100" className="w-48 h-48 animate-pie-in">
             ...
             <style>{`
                ...
                .animate-pie-in { animation: pie-in 0.8s cubic-bezier(0.25, 1, 0.5, 1) forwards; }
            `}</style>
...
const LineChart = ({ data, color }: { data: number[], color: string }) => {
    if (!data || data.length === 0) return <div className="h-full flex items-center justify-center text-gray-500">Sem dados para exibir</div>;
    
    const maxVal = Math.max(...data);
    const points = data.map((val, i) => `${(i / (data.length - 1)) * 100},${100 - (val / maxVal) * 90}`).join(' ');
    ...
    return (
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full">
            ...
            <style>{`
                ...
                .animate-line-draw { animation: line-draw 1.5s ease-in-out forwards; }
            `}</style>
        </svg>
    );
};
...
const DashboardView = ({ data }: { data: AdminData }) => {
    ...
    const revenueData = data.transactions.map(t => t.value);
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                 <div className="bg-white p-6 rounded-lg shadow"><h3 className="font-bold mb-4">Receita (Últimos 6 Meses)</h3><div className="h-64"><LineChart data={revenueData} color="#3B82F6"/></div></div>
...
const UsersView = ({ data, setData }: { data: AdminData['users'], setData: React.Dispatch<React.SetStateAction<AdminData>> }) => {
    ...
    const handleDelete = (userId: number) => {
        if(window.confirm('Tem certeza que deseja remover este usuário? Esta ação é irreversível.')) {
             ...
             addToast('Usuário removido.', 'error');
        }
    };
                        ...
                        {filteredUsers.length > 0 ? filteredUsers.map(user => (
                            <tr key={user.id} className="border-b hover:bg-gray-50">
                                ...
                                <td className="space-x-3">
                                    ...
                                    <button onClick={() => handleDelete(user.id)} className="text-red-600 hover:underline text-sm font-medium">Remover</button>
                                </td>
                            </tr>
                        )) : (
...
const VaquinhasView = ({ data, setData }: { data: AdminData['vaquinhas'], setData: React.Dispatch<React.SetStateAction<AdminData>> }) => {
    const { addToast } = useToast();
    const handleToggleSuspend = (vaquinhaId: number) => {
        setData(prev => ({
            ...prev,
            vaquinhas: prev.vaquinhas.map(v => {
                if(v.id === vaquinhaId) {
                    const newStatus = v.status === 'Ativa' ? 'Suspensa' : 'Ativa';
                    addToast(`Vaquinha ${newStatus.toLowerCase()} com sucesso!`);
                ...
                <tbody>
                    {data.map(v => (
                        <tr key={v.id} className="border-b hover:bg-gray-50">
                            ...
                            <td><span className={`px-2 py-1 text-xs rounded-full ${v.status === 'Ativa' ? 'bg-green-100 text-green-700' : v.status === 'Finalizada' ? 'bg-gray-200 text-gray-700' : v.status === 'Suspensa' ? 'bg-orange-100 text-orange-700' : 'bg-red-100 text-red-700'}`}>{v.status}</span></td>
                            <td>R$ {v.collected.toLocaleString('pt-BR')}</td>
                            <td className="space-x-2"><button className="text-blue-600 hover:underline text-sm font-medium">Visualizar</button><button onClick={() => handleToggleSuspend(v.id)} className="text-red-600 hover:underline text-sm font-medium">{v.status === 'Ativa' ? 'Suspender' : 'Reativar'}</button></td>
                        </tr>
                    ))}
                </tbody>
            </table>
...
const BarChart = ({ data, color }: { data: {label: string, value: number}[], color: string }) => {
     ...
     return (
        <div className="h-full w-full flex justify-around items-end p-4 border-l border-b border-gray-200">
            {data.map((item, index) => (
                <div key={index} className="flex flex-col items-center flex-1">
                    <div 
                        className="w-1/2 bg-blue-500 rounded-t-md animate-bar-grow" 
                        style={{ height: `${(item.value / maxVal) * 100}%`, backgroundColor: color, animationDelay: `${index * 100}ms` }}
                    />
             ...
             <style>{`
                ...
                .animate-bar-grow { animation: bar-grow 0.8s cubic-bezier(0.25, 1, 0.5, 1) forwards; }
            `}</style>
        </div>
     );
};
...
const FinanceView = ({ data }: { data: AdminData['transactions'] }) => {
    const revenueByModel = data.reduce((acc, t) => {
        acc[t.type] = (acc[t.type] || 0) + t.value;
        return acc;
    }, {} as Record<string, number>);

    const barChartData = Object.entries(revenueByModel).map(([label, value]) => ({ label, value }));
    
    return (
    <div>
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Financeiro</h1>
        <div className="bg-white p-6 rounded-lg shadow mb-6"><h3 className="font-bold mb-4">Receita por Modelo</h3><div className="h-64"><BarChart data={barChartData} color="#10B981" /></div></div>
        <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="font-bold mb-4">Transações Recentes</h3>
             <table className="w-full text-left">
                ...
                <tbody>
                    {data.slice().reverse().map(t => (
                        <tr key={t.id} className="border-b hover:bg-gray-50">
                            <td className="p-3">{t.date}</td><td>R$ {t.value.toLocaleString('pt-BR')}</td><td>{t.type}</td><td><span className="text-green-700">Completo</span></td>
                        </tr>
                    ))}
                </tbody>
            </table>
        ...
        <div>
            ...
            <div className="bg-white p-6 rounded-lg shadow space-y-6 max-w-2xl">
                ...
                <div>
                    ...
                    <div className="flex items-center space-x-4">
                        <label className="flex items-center"><input type="checkbox" name="mercadoPago" checked={config.mercadoPago} onChange={handleChange} className="mr-2 h-4 w-4 text-emerald-600 border-gray-300 rounded"/> Mercado Pago</label>
                        <label className="flex items-center"><input type="checkbox" name="picPay" checked={config.picPay} onChange={handleChange} className="mr-2 h-4 w-4 text-emerald-600 border-gray-300 rounded"/> PicPay</label>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody>
                             {data.length > 0 ? data.map(l => (
                               ...
                               <tr><td colSpan={5} className="text-center py-8 text-gray-500">Nenhum licenciado cadastrado.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                    ...
                    {data.map(t => (
                        <tr key={t.id} className="border-b hover:bg-gray-50">
                            ...
                            <td className="space-x-2">
                                <button onClick={() => addToast('Funcionalidade de resposta em desenvolvimento.')} disabled={t.status === 'Fechado'} className="text-blue-600 hover:underline text-sm font-medium disabled:text-gray-400 disabled:no-underline">Responder</button>
